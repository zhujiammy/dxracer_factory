...
public class Durable extends AppCompatActivity {

    private ColumnChartView columnChartView,columnChartView1,columnChartView2,columnChartView3,columnChartView4;
    private ColumnChartData data,data1,data2,data3,data4;
    ...
    private List<ConsumableData> consumableData3=new ArrayList<>();
    private List<ConsumableData> consumableData4=new ArrayList<>();
    @SuppressLint("WrongConstant")
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.durable);
    ...
    private void loaddata(){
        ...
        HttpUtility.doPostAsyn("/servlet/officegoods/chart/durable", params, new IHttpCallBack() {
            @Override
    ...
    private void initView(){
        ...
        columnChartView3=(ColumnChartView)findViewById(R.id.chart3);
        columnChartView3.setZoomEnabled(false);
        columnChartView4=(ColumnChartView)findViewById(R.id.chart4);
            ...
            try {
                switch (msg.what){
                    case 1:
                        ...
                        //当前物品总价值
                        JSONArray totalPriceChart=object.getJSONArray("totalPriceChart");

                        for(int i=0;i<totalPriceChart.length();i++){
                            JSONObject object1=totalPriceChart.getJSONObject(i);
                            consumableData3.add(new ConsumableData(object1.getString("totalPrice"),object1.getString("durableType")));
                        }
                        int numSubcolumns3=1;
                        int numColumns3=consumableData3.size();
                        Log.e("TAG", "initData: "+consumableData3.size() );
                        List<AxisValue>axisValues4=new ArrayList<>();
                        List<Column>columns3=new ArrayList<>();
                        List<SubcolumnValue>values3;

                        for(int i=0;i<numColumns3;i++){
                            values3=new ArrayList<>();
                            for(int j=0;j<numSubcolumns3;j++){
                                values3.add(new SubcolumnValue(Float.parseFloat(consumableData3.get(i).getNum()),
                                        ChartUtils.pickColor()));
                            }
                            //点击柱状图就展示数据
                            columns3.add(new Column(values3).setHasLabels(true).setHasLabelsOnlyForSelected(true));
                            axisValues4.add(new AxisValue(i).setLabel(consumableData3.get(i).getText()));
                        }
                        Log.e("TAG", "handleMessage: "+consumableData3.get(0).getText());

                        data3=new ColumnChartData(columns3);
                        data3.setAxisXBottom(new Axis(axisValues4).setHasLines(true).setTextColor(Color.BLACK).setHasTiltedLabels(true).setTextSize(10).setInside(false));
                        data3.setAxisYLeft(new Axis().setHasLines(true).setTextColor(Color.BLACK).setMaxLabelChars(4));
                        columnChartView3.setColumnChartData(data3);



                        //当前物品总数量
                        JSONArray totalNumChart=object.getJSONArray("totalNumChart");

                        for(int i=0;i<totalNumChart.length();i++){
                            JSONObject object1=totalNumChart.getJSONObject(i);
                            consumableData4.add(new ConsumableData(object1.getString("num"),object1.getString("durableType")));
                        }
                        int numSubcolumns4=1;
                        int numColumns4=consumableData4.size();
                        Log.e("TAG", "initData: "+consumableData4.size() );
                        List<AxisValue>axisValues5=new ArrayList<>();
                        List<Column>columns4=new ArrayList<>();
                        List<SubcolumnValue>values4;

                        for(int i=0;i<numColumns4;i++){
                            values4=new ArrayList<>();
                            for(int j=0;j<numSubcolumns4;j++){
                                values4.add(new SubcolumnValue(Float.parseFloat(consumableData4.get(i).getNum()),
                                        ChartUtils.pickColor()));
                            }
                            //点击柱状图就展示数据
                            columns4.add(new Column(values4).setHasLabels(true).setHasLabelsOnlyForSelected(true));
                            axisValues5.add(new AxisValue(i).setLabel(consumableData4.get(i).getText()));
                        }
                        Log.e("TAG", "handleMessage: "+consumableData4.get(0).getText());

                        data4=new ColumnChartData(columns4);
                        data4.setAxisXBottom(new Axis(axisValues5).setHasLines(true).setTextColor(Color.BLACK).setHasTiltedLabels(true).setTextSize(10).setInside(false));
                        data4.setAxisYLeft(new Axis().setHasLines(true).setTextColor(Color.BLACK).setMaxLabelChars(4));
                        columnChartView4.setColumnChartData(data4);
                        break;
