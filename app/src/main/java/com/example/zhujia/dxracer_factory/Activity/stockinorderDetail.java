...
public class stockinorderDetail extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if(v==stockinorderitem){
            //入库单详情
            intent=new Intent(this,Stockinorderitems.class);
            intent.putExtra("stockInOrderId",stockInOrderId);
            startActivity(intent);
        }
        if(v==stockinorderlog){
            //操作日志
            intent=new Intent(this,Stockinorderlogs.class);
            intent.putExtra("stockInOrderId",stockInOrderId);
            startActivity(intent);
        }
    }
